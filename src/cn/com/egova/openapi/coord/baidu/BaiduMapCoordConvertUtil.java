...
/**
 * 百度地图坐标转换工具，根据Gis组提供的js文件改写
 * 主要提供两种转换方法：经纬度转平面(米制坐标)， 平面(米制坐标)转经纬度
 * @author zhw
 * @2015-6-18
 */
public class BaiduMapCoordConvertUtil {
    ...
    private static MyLatLng convertor(MyLatLng t, double[] e) {
        if (t != null && e != null) {
            double i = e[0] + e[1] * Math.abs(t.longitude),
                n = Math.abs(t.latitude) / e[9],
                a = e[2] + e[3] * n + e[4] * n * n + e[5] * n * n * n + e[6] * n * n * n * n + e[7] * n * n * n * n * n + e[8] * n * n * n * n * n * n;
            i *= t.longitude < 0 ? -1 : 1;
            a *= t.latitude < 0 ? -1 : 1;
            return new MyLatLng(a, i);
        }
    ...
    //平面坐标(米制坐标)转经纬度坐标
    public static MyLatLng convertMC2LL(MyLatLng t) {
        MyLatLng e = new MyLatLng(Math.abs(t.latitude), Math.abs(t.longitude));
        double[] i = null;
        for (int n = 0; n < MCBAND.length; n++) {
            if (e.latitude >= MCBAND[n]) {
                i = MC2LL[n];
                break;
            }
        }
        return convertor(t, i);
    }
    
    //经纬度转平面(米制坐标)
    public static MyLatLng convertLL2MC(MyLatLng t) {
        MyLatLng e;
        double[] i = null;
        t.longitude = getLoop(t.longitude, -180, 180);
        t.latitude = getRange(t.latitude, -74, 74);
        e = t;
        for (int n = 0; n < LLBAND.length; n++) {
            if (e.latitude >= LLBAND[n]) {
                i = LL2MC[n];
                break;
            }
        }
